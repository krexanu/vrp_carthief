vRPcarthiefC = {}
Tunnel.bindInterface("vRP_carthief",vRPcarthiefC)
Proxy.addInterface("vRP_carthief",vRPcarthiefC)
vRP = Proxy.getInterface("vRP")
vRPScarthief = Tunnel.getInterface("vRP_carthief","vRP_carthief")
...
local cirklar = { 
	["~g~[E] ~w~ Incepe"] = { ["x"] = -2172.95, ["y"] = 4284.63, ["z"] = 48.27, ["mission"] = 0}
} 
...
Citizen.CreateThread(function()
	while true do
		...
		vRPCarthief.TriggerServerCallback('loffe_carthief:getTimeLeft', function(timeleft)
			if timeleft > 0 then
			...
			if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 100 then
				...
				if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 1.5 then
					...
					if IsControlJustPressed(1, 51) then
						vRPCarthief.TriggerServerCallback('vezijob', function(cb)
							if cb then
			...
			if GetDistanceBetweenCoords(coords, Config.dropoffPoints[dropoff].sx, Config.dropoffPoints[dropoff].sy, Config.dropoffPoints[dropoff].sz, true) < 50 then
				...
				repeat
					...
					if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
						...
						ClearPrints()
						SetTextEntry_2("STRING")
						AddTextComponentString('Hot wire')
						DrawSubtitleTimed(500, 1)
						Citizen.Wait(500)
						ClearPrints()
						SetTextEntry_2("STRING")
				...
				if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
					ClearPrints()
					SetTextEntry_2("STRING")
					AddTextComponentString('Semnal '.. timesUntilSendareLoss)
